...
describe("integration tests", function () {
  ...
  const HORIZON = 'https://orbit-testnet.rover.network';
  RoverSdk.Network.useTestNetwork();
  let server = new RoverSdk.Server(ORBIT);
  let master = RoverSdk.Keypair.random();

  before(function(done) {
    axios.get(`${ORBIT}/dakibot?addr=${master.publicKey()}`).then(() => done());
  });
    ...
    server.operations().forAccount(master.publicKey()).limit(1).order('asc')
      ...
      .then(response => {
        ...
        return server.loadAccount(master.publicKey())
          .then(source => {
            let tx = new RoverSdk.TransactionBuilder(source)
              .addOperation(RoverSdk.Operation.accountMerge({
  ...
  describe("/transaction", function () {
    it("submits a new transaction", function (done) {
      createNewAccount(RoverSdk.Keypair.random().publicKey())
        .then(result => {
    ...
    it("submits a new transaction with error", function (done) {
      server.loadAccount(master.publicKey())
        .then(source => {
          ...
          let tx = new RoverSdk.TransactionBuilder(source)
            .addOperation(RoverSdk.Operation.createAccount({
              destination: RoverSdk.Keypair.random().publicKey(),
              startingBalance: "1"
            }))
            .build();
  ...
  describe("/accounts", function () {
    ...
    it("stream accounts", function (done) {
      ...
      let randomAccount = RoverSdk.Keypair.random();
