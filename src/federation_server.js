...
import {Account, StrKey} from 'rover-base';
import {BadResponseError} from './errors';
import {RoverTomlResolver} from "./rover_toml_resolver";
 ...
 * [federation server](https://www.rover.network/developers/guides/concepts/federation.html)
...
export class FederationServer {
   ...
   *
   * * For Rover address (ex. `bob*rover.network`) it splits Rover address and then tries to find information about
   * federation server in `rover.toml` file for a given domain. It returns a `Promise` which resolves if federation
   ...
   *
   * @see <a href="https://www.rover.network/developers/guides/concepts/federation.html" target="_blank">Federation doc</a>
   * @see <a href="https://www.rover.network/developers/guides/concepts/rover-toml.html" target="_blank">Rover.toml doc</a>
   * @param {string} value Rover Address (ex. `bob*rover.network`)
   * @param {object} [opts]
  ...
  static resolve(value, opts = {}) {
    ...
    if (value.indexOf('*') < 0) {
      ...
      if (addressParts.length != 2 || !domain) {
        return Promise.reject(new Error('Invalid Rover address'));
      }
  ...
  /**
   * Creates a `FederationServer` instance based on information from [rover.toml](https://www.rover.network/developers/guides/concepts/rover-toml.html) file for a given domain.
   * Returns a `Promise` that resolves to a `FederationServer` object. If `rover.toml` file does not exist for a given domain or it does not contain information about a federation server Promise will reject.
   * ```js
   ...
   *   })
   *   .catch(error => {
   *     // rover.toml does not exist or it does not contain information about federation server.
   *   });
   * ```
   * @see <a href="https://www.rover.network/developers/guides/concepts/rover-toml.html" target="_blank">Rover.toml doc</a>
  ...
  static createForDomain(domain, opts = {}) {
    return RoverTomlResolver.resolve(domain)
      .then(tomlObject => {
        if (!tomlObject.FEDERATION_SERVER) {
          return Promise.reject(new Error('rover.toml does not contain FEDERATION_SERVER field'));
        }
  ...
  /**
   * Returns a Promise that resolves to federation record if the user was found for a given Rover address.
   * @see <a href="https://www.rover.network/developers/guides/concepts/federation.html" target="_blank">Federation doc</a>
   * @param {string} address Rover address (ex. `bob*rover.network`). If `FederationServer` was instantiated with `domain` param only username (ex. `bob`) can be passed.
   * @returns {Promise}
   */
  resolveAddress(address) {
    if (address.indexOf('*') < 0) {
      if (!this.domain) {
        return Promise.reject(new Error('Unknown domain. Make sure `address` contains a domain (ex. `bob*rover.network`) or pass `domain` parameter when instantiating the server object.'));
      }
